...
export default function PhotographyContent() {
  ...
  useEffect(() => {
    async function fetchCollections() {
      try {
        ...
        // Fetch collections for photography
        const response = await fetch(`/api/visual?type=photography`);
        ...
        console.error('Error fetching photography collections:', error);
      } finally {
  ...
  useEffect(() => {
    async function fetchCollectionImages() {
      ...
      try {
        ...
        const response = await fetch(`/api/visual?type=photography&collectionId=${activeCollection}`);
        const data = await response.json();
  ...
  // Open image modal with the clicked image - manages modal visibility state
      ...
      <div className="folder-content">
        ...
        {collections.length === 0 && !loadingCollections ? (
          ...
          <div className="folder-empty flex flex-col items-center justify-center py-16">
            ...
            <p className="text-gray-600 text-center max-w-md">Photography collections are being prepared and will be available soon.</p>
          </div>
        ) : !activeCollection ? (
